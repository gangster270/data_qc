...
### 3.1-1 조사일 기준을 직접 정하기

생육 자료가 아직 없어도 **원하는 조사일 기준**만 정하면 구간 환경을 뽑을 수 있다.

| 방식 | CLI | 쓰임 |
|---|---|---|
| 시작일 + 간격 + 횟수 | `--survey-start 2026-04-01 --survey-interval 10 --survey-count 6` | 정기 조사 계획이 정해진 경우 |
| 시작일 + 간격 + 종료일 | `--survey-start 2026-04-01 --survey-interval 7 --survey-end 2026-06-08` | 작기 전체를 한 번에 |
| 조사일 직접 나열 | `--survey-dates "2026-04-01,2026-04-11,2026-04-18"` | **실제 조사일이 불규칙할 때(현장 사정으로 밀린 날짜 그대로)** |

우선순위는 `사용자 지정 > 생육 파일`이다. 생육 파일을 함께 주면 값까지 병합되고,
조사일만 주면 `env_interval_summary.csv`(구간 환경 요약)까지 생성된다.
대시보드에서는 🔁 전처리 탭 **② 조사일 기준 정하기**에서 같은 세 방식을 고른다.
